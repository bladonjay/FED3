void makeFile() {
  if (saveOpt == true) {
    ...
    if ( ! logfile ) {
      ...
      logfile.println("AdaptaMaze File created");
      logfile.println("date in MM:DD:YYYY hh:mm:ss, Task Paradigm");
      logfile.print(filename[0]);
      logfile.print(filename[1]);
      logfile.print("/");
      logfile.print(filename[2]);
      logfile.print(filename[3]);
      logfile.print("/20");
      logfile.print(filename[4]);
      logfile.print(filename[5]);
      logfile.print(" ");
      thistime = millis();
...
void writeData() {
  ...
  int thishour = (thistime - starttime) / 3600000;
  int thisminute = ((thistime - starttime) - (thishour * 3600000)) / 60000;
  int thissecond = ((thistime - starttime) - (thishour * 3600000) - (thisminute * 60000)) / 10;
  mydateHMS[0] = thishour / 10 + '0';
  mydateHMS[1] = thishour % 10 + '0';
  mydateHMS[3] = thisminute / 10 + '0';
  mydateHMS[4] = thisminute % 10 + '0';
  mydateHMS[6] = thissecond / 1000 + '0';
  mydateHMS[7] = thissecond % 1000 + '0';
  mydateHMS[8] = thissecond / 10 + '0';
  mydateHMS[9] = thissecond % 10 + '0';
  logfile.print(mydateHMS);
  logfile.print(",");
